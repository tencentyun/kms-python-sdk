...
class KMSHTTPConnection(HTTPConnection):
...
class KMSHTTPSConnection(HTTPSConnection):
    def __init__(self, host, port=None, strict=None):
...
class KMSHttp:
    def __init__(self, host, connection_timeout=10, keep_alive=True, logger=None, is_https=False):
        if is_https:
            self.conn = KMSHTTPSConnection(host)
        else:
            self.conn = KMSHTTPConnection(host)
        ...
        self.logger = logger
        if self.logger:
            self.logger.debug("InitKMSHttp KeepAlive:%s ConnectionTime:%s" % (self.keep_alive, self.connection_timeout))
    ...
    def send_request(self, req_inter):
        try:
            ...
            try:
                ...
                # raise KMSClientNetworkException("Connection TImeout");
                self.conn.request(req_inter.method, req_inter.uri, req_inter.data, req_inter.header)
                self.conn.sock.settimeout(self.connection_timeout)
                self.conn.sock.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
                http_resp = self.conn.getresponse()
            ...
            self.conn.close()
            raise KMSClientNetworkException(str(e))
